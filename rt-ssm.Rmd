...
I have mentioned this in the margin notes twice, but the other change we'll make is to call $\gamma(R_t-1) = \theta$. 
...
Then a state space model with just an intercept and `offset` $= I_t$ will be:
...
```{r fig.fullwidth=T, fig.height=5}
rt %>% 
...
The main difference, however, is that the uncertainty in this model never collapses as far as it does in Systrom's model. This is due to the fact that we are assuming that $\gamma(R_t-1)$ (and by extension, $R_t$) is a moving target. 
